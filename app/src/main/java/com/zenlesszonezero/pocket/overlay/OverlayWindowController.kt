...
import com.zenlesszonezero.pocket.feature.autoskip.AutoSkipEvents
import com.zenlesszonezero.pocket.genshin.GenshinLaunchResult
import com.zenlesszonezero.pocket.genshin.GenshinLauncher
import com.zenlesszonezero.pocket.genshin.GenshinPackages
import com.zenlesszonezero.pocket.input.AccessibilityServiceHealth
...
class OverlayWindowController(
    ...
    private fun launchGenshinFromButton() {
        when (genshinLauncher.launch()) {
            ...
            GenshinLaunchResult.NotInstalled -> {
                Toast.makeText(themedContext, "未安装绝区零", Toast.LENGTH_SHORT).show()
                refreshLaunchHint()
            }
            is GenshinLaunchResult.Failed -> {
                Toast.makeText(themedContext, "无法启动绝区零", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun refreshLaunchHint() {
        ...
        if (pkg != null) {
            ...
            launchSubtitle?.text = "未安装绝区零"
            launchHint?.text = "未安装绝区零"
        }
    }
